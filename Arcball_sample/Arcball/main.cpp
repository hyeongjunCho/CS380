...
glm::mat4 objectRBT = glm::translate(glm::mat4(1.0f), glm::vec3(1.0f, 1.0f, 0.0f));
...
int main(void)
{
	...
	window = glfwCreateWindow((int)windowWidth, (int)windowHeight, "TA Solution", NULL, NULL);
	if (window == NULL) {
		glfwTerminate();
		return -1;
	}
	...
	object = Model();
	init_cube(object, glm::vec3(1.0, 0.0, 0.0));
	...
	do {
		...
		// Set a new transformation into each model
