...
void main(){
	...
	else
	{
		...
		campos = (Eye * vec4(WorldCameraPosition, 1.0)).xyz;
		//campos = (vec4(WorldCameraPosition, 1.0)).xyz;
		vec3 worldView = normalize(campos - worldPos);

		Normal = mat3(transpose(inverse(ModelTransform))) * normalize(vertexNormal_modelspace);
		Position = worldPos;
